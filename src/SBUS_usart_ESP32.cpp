...
void SBUS2_uart_setup()
{
   ...
   ESP_ERROR_CHECK(uart_set_line_inverse(UART_NUM_1, UART_INVERSE_RXD|UART_INVERSE_TXD)); 
   ESP_ERROR_CHECK(uart_set_pin(UART_NUM_1, SBUS2_UART_TX_PIN, SBUS2_UART_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
   gpio_set_pull_mode(SBUS2_UART_RX_PIN, GPIO_PULLDOWN_ONLY);
   gpio_pulldown_en(SBUS2_UART_RX_PIN);
   ESP_ERROR_CHECK(uart_driver_install(UART_NUM_1, RX_BUF_SIZE, 0,0, NULL, 0));
   ESP_ERROR_CHECK(uart_isr_free(UART_NUM_1));
   ESP_ERROR_CHECK(uart_isr_register(UART_NUM_1,uart_intr_handle, NULL, ESP_INTR_FLAG_IRAM, 0/*handle_console*/));
...
void SBUS2_get_status( uint16_t *uart_dropped_frame, bool *transmision_dropt_frame, bool *failsave )
{
   ...
   *transmision_dropt_frame = rxbuf[23] & 0x04 ? true : false;
   *failsave = rxbuf[23] & 0x08 ? true : false;
}
